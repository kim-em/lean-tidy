...
by rewrite_search {explain := tt, trace_summary := tt, no visualiser, metric.edit_distance {}}

lemma test4_ring : (a * (b + c + 1)) * d = a * (b * d) + a * (1 * d) + a * (c * d) :=
by ring

lemma test4 : (a * (b + c + 1)) * d = a * (b * d) + a * (1 * d) + a * (c * d) :=
by rewrite_search {explain := tt, trace_summary := tt, no visualiser, metric.edit_distance {refresh_freq := 3} cm, strategy := pexplore, max_iterations := 100}
