...
by rewrite_search {explain := tt, trace_summary := tt, view := no visualiser, metric := edit_distance}

lemma test4_ring : (a * (b + c + 1)) * d = a * (b * d) + a * (1 * d) + a * (c * d) :=
by ring

lemma test4 : (a * (b + c + 1)) * d = a * (b * d) + a * (1 * d) + a * (c * d) :=
by rewrite_search {explain := tt, trace_summary := tt, view := no visualiser, metric := edit_distance {refresh_freq := 3} weight.cm, strategy := pexplore, max_iterations := 100}
...
-- by rewrite_search [add_comm, add_assoc, mul_one, mul_assoc, /-mul_comm,-/ left_distrib, right_distrib] {explain := tt, trace_summary := tt, view := no visualiser, metric := edit_distance {refresh_freq := 3} weight.svm, strategy := bfs, max_iterations := 2000}
