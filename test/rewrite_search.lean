...
-- nth_rewrite_rhs 1 ←qux'',
  rewrite_search {view := no visualiser}, -- FIXME this is broken until we can do single replacements.
end
...
-- by rewrite_search_with [add_comm, add_assoc, mul_one, mul_assoc, mul_comm, left_distrib, right_distrib] {explain := tt, trace_summary := tt, view := visualiser, metric := edit_distance {refresh_freq := 10} weight.cm, strategy := pexplore {pop_size := 3}, max_iterations := 1000}
