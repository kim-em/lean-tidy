...
meta def stateful_tactic (β : Type) (α : Type) := interaction_monad (tactic_state × β) α

meta def profiling_tactic := stateful_tactic profiling_state

meta def tactic_pure { α : Type } ( a : α ) : tactic α := pure a
meta def tactic_state_pure { α : Type } ( a : α ) ( ts : tactic_state ) : tactic_state :=
begin
  have p := tactic_pure a ts,
  induction p with _ ts' _ _ ts'',
  exact ts',
  exact ts''
end
meta def tactic_pure_lemma { α : Type } ( a : α ) ( ts : tactic_state ) : tactic_pure a ts = result.success a (tactic_state_pure a ts) :=
begin
  dunfold tactic_state_pure,
  dunfold tactic_pure,
  dunfold pure,
  dunfold tactic,
  dunfold interaction_monad,
  dunfold has_pure.pure,
  dunfold interaction_monad_return,
  dsimp,
  refl  
end
...
meta instance profiling_tactical_monad : tactical_monad profiling_tactic := { 
  pure   := λ { α : Type } ( a : α ) ( s : tactic_state × profiling_state ), 
              result.success a (tactic_state_pure a s.1, ⟨ s.2.invocations + 1 ⟩),
  bind   := λ { α β : Type } ( t : profiling_tactic α ) ( f : α → profiling_tactic β ) ( s : tactic_state × profiling_state ), 
              match (t s) with
              | result.success a s' := f a s'
              | result.exception pos msg s' := result.exception pos msg s'
              end,
  lift   := λ { α : Type } ( t : tactic α ) ( s : tactic_state × profiling_state ), 
              match (t s.1) with
  ...
  fail   := λ { α : Type } (msg : string), begin admit end,
  orelse := λ { α : Type } ( t₁ : profiling_tactic α ) ( t₂ : profiling_tactic α ) ( s : tactic_state × profiling_state ), 
              match (t₁ s) with
